...
BEGIN
    batch_count := CEIL(total_records::FLOAT / batch_size);
    RAISE NOTICE 'バッチ処理開始: 総件数 %, バッチサイズ %, バッチ数 %', total_records, batch_size, batch_count;
    
    FOR current_batch IN 1..batch_count LOOP
        start_idx := (current_batch - 1) * batch_size + 1;
        end_idx := LEAST(current_batch * batch_size, total_records);
        
        RAISE NOTICE 'バッチ % / % 処理中: % - % 件目', current_batch, batch_count, start_idx, end_idx;
        ...
        PERFORM pg_sleep(0.1);  -- 短い休憩でデータベース負荷を軽減
        
    END LOOP;
    
    RAISE NOTICE 'テストデータ生成完了: %件のユーザーデータを作成しました', total_records;
END $$;
