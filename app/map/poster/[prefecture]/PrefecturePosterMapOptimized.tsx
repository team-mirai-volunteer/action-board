...
// Dynamic import for the optimized map
const PosterMapOptimized = dynamic(() => import("../PosterMapOptimized"), {
  ssr: false,
  loading: () => (
...
interface PrefecturePosterMapOptimizedProps {
  userId?: string;
  prefecture: string;
...
export default function PrefecturePosterMapOptimized({
  userId,
  prefecture,
  prefectureName,
  center,
  initialBoards,
  initialStats,
}: PrefecturePosterMapOptimizedProps) {
  ...
  const handleStatusUpdate = async () => {
    ...
    try {
      ...
      if (updateStatus === "done") {
        ...
        if (user) {
          // ミッション達成処理を実行
          await completePosterBoardMission(selectedBoard);
        }
      }
  ...
  const completePosterBoardMission = async (board: PosterBoard) => {
    const supabase = createClient();
