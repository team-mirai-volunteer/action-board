...
export default function PosterMap({ prefecture, user }: PosterMapProps) {
    ...
    const fetchPins = async () => {
      try {
        ...
        const response = await fetch(
          `/api/poster-map/pins?prefecture=${encodeURIComponent(prefecture)}`,
        );
        if (!response.ok) {
  ...
  useEffect(() => {
    ...
    if (mapInstance.getZoom() === undefined) {
      mapInstance.setView(getPrefectureCoordinates(prefecture), 8);
    }
    ...
    const drawPins = async () => {
      ...
      const newControl = L.control
        .layers({}, overlaysRef.current as unknown as { [key: string]: Layer })
        .addTo(mapInstance);
    ...
    drawPins();
  }, [mapInstance, pins, prefecture, loading]);
