...
- `subject`: 選挙の種類（enum型）
  ...
  - "都道府県首長選"
  - "市区町村首長選"
- `municipal_codes`: 対象となる地方公共団体コードの配列
  - 全国規模の選挙の場合は空配列
  - 地方選挙の場合は該当する地方公共団体コードを格納
...
VALUES (
  season1_id,
  '2025-06-24 00:00:00+09',  -- 選挙期間開始
  '2025-07-13 20:00:00+09',  -- 選挙期間終了（投票日20時）
  '参院選',
  '{}'  -- 全国選挙
);
```
...
`src/lib/types/supabase.ts`に以下の型を追加:
...
`supabase/migrations/20251230223552_add_elections_table.sql`
...
管理者が選挙を作成・編集・削除できる管理画面を追加することで、より柔軟な運用が可能になります。
